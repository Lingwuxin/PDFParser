...
def compare_standard_toc(self):
    ...
    # 检查是否有section不在标准section列表中
    for this_item in self.sections_compared_std:
        if type(this_item[6])!=int:
            self.not_in_std_sections.append((this_item[0], this_item[1], this_item[2]))
    if len(self.not_in_std_sections)>0:
        no_warning=0
        print(f"Warning from compare_standard_toc: the following sections are not in standard sections:")
        print(f"{self.not_in_std_sections}")


    #如果section与标准section列表完全一致：
    if no_warning==1:
